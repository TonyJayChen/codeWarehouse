...
public final class ImageUtils {
     ...
     * @param alpha             水印透明度
     */
    public static void addWatermark(File srcFile, File destFile, File watermarkFile, ImageUtils.WatermarkPosition watermarkPosition, int alpha) {
        Assert.notNull(srcFile, "[Assertion failed] - srcFile is required; it must not be null");
        Assert.state(srcFile.exists(), "[Assertion failed] - srcFile must exists");
        Assert.state(srcFile.isFile(), "[Assertion failed] - srcFile must be file");
        Assert.notNull(destFile, "[Assertion failed] - destFile is required; it must not be null");
        ...
        if (watermarkFile == null || !watermarkFile.exists() || !watermarkFile.isFile() || watermarkPosition == null || ImageUtils.WatermarkPosition.NO.equals(watermarkPosition)) {
            try {
